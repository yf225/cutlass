...
class FlopCounterMode:
    ...
    def get_table(self, depth=None):
        ...
        header = ["Module", "FLOP", "% Total", "Shapes & Types"]
        values = []
        ...
        def process_mod(mod_name, depth):
            ...
            for k, v in self.flop_counts[mod_name].items():
                # First add the total for this operation
                op_percent = v / global_flops
                values.append([
                    padding + " - " + str(k),
                ...
                # Then add each shape+dtype combination and its individual flop count
                shape_flops = self.shape_counts[mod_name][k]
                shape_percents_sum = sum(shape_flop / global_flops for shape_flop in shape_flops.values())
                # Assert that shape percentages sum up to operation percentage (within floating point error)
                assert abs(shape_percents_sum - op_percent) < 1e-6, \
                    f"Shape percentages {shape_percents_sum:.6f} don't sum to operation percentage {op_percent:.6f} for {k}"
                
                # Sort shapes by flop count in descending order
                sorted_shapes = sorted(shape_flops.items(), key=lambda x: x[1], reverse=True)
                
                for shape_dtype_str, shape_flop in sorted_shapes:
                    values.append([
                        padding + "   ",  # Extra indent for shape details
                        convert_num_with_suffix(shape_flop, global_suffix),
                        convert_to_percent_str(shape_flop, global_flops),
                        shape_dtype_str
                    ])
            return values
        ...
        return tabulate.tabulate(values, headers=header, colalign=("left", "right", "right", "left"))
    ...
    def _count_flops_and_shapes(self, func_packet, out, args, kwargs):
        if func_packet in self.flop_registry:
            flop_count_func = self.flop_registry[func_packet]
            flop_count = flop_count_func(*args, **kwargs, out_val=out)

            # Get shapes, strides and dtypes of tensor arguments
            shapes_strides_and_dtypes = []
            for arg in args:
                if isinstance(arg, torch.Tensor):
                    shapes_strides_and_dtypes.append((
                        tuple(arg.shape),
                        tuple(arg.stride()),
                        str(arg.dtype)
                    ))
                elif isinstance(arg, (list, tuple)):
                    sub_shapes_strides_dtypes = []
                    for sub_arg in arg:
                        if isinstance(sub_arg, torch.Tensor):
                            sub_shapes_strides_dtypes.append((
                                tuple(sub_arg.shape),
            ...
            for par in set(self.mod_tracker.parents):
                ...
                # Track flops per shape, stride and dtype combination
                self.shape_counts[par][func_packet][shape_stride_dtype_key] += flop_count
...
class _FlopCounterMode(TorchDispatchMode):
    ...
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        # no further decomposition; execute & count flops and shapes
        out = func(*args, **kwargs)
        return self.counter._count_flops_and_shapes(func._overloadpacket, out, args, kwargs)
