...
class ArrayEngine:
  def __new__(_, T: cls, N: size_t):
    class _inner:
      Storage = typename(conditional((sizeof_bits.spec(T).value % 8 == 0),
                            array_aligned.spec(T, N),
                            array_subbyte.spec(T, N)).type)
      iterator = typename(Storage.iterator)
      reference = typename(iterator_traits.spec(iterator).reference)
      element_type = typename(iterator_traits.spec(iterator).element_type)
      value_type = typename(iterator_traits.spec(iterator).value_type)
      storage_ = Storage()

      @attrs(CUTE_HOST_DEVICE, constexpr, const)
      def begin(self):
          return self.storage_.begin()

      @attrs(CUTE_HOST_DEVICE, constexpr)
      def begin(self):
          return self.storage_.begin()
...
class ArrayEngine:
  def __new__(_, S: int, T: cls, N: size_t):
    @spec(sparse_elem.spec(S, T), N)
    class _inner:
      static_assert(N % S == 0, "Expected a multiple of the sparsity.")
      value_type = sparse_elem.spec(S, T)
      Storage = typename(conditional((sizeof_bits.spec(T).value % 8 == 0),
                                    array_aligned.spec(T, N / S),
                                    array_subbyte.spec(T, N / S)).type)
      iterator = sparse_ptr.spec(S, Ptr[sparse_elem.spec(S, T)])
      reference = typename(iterator_traits.spec(iterator).reference)
      element_type = typename(iterator_traits.spec(iterator).element_type)
      storage_ = Storage()
      ...
      @attrs(CUTE_HOST_DEVICE, constexpr, const)
      def begin(self):
          return recast_ptr.spec(value_type)(self.storage_.begin())
